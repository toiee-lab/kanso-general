 ...
 * badge
 */
add_shortcode( 'uk-badge' , function( $atts, $content ){
 ...
 * button
 */
add_shortcode( 'uk-button' , function ( $atts , $content ){
 ...
 * icon
 */
add_shortcode( 'uk-label' , function( $atts, $content ){
	$atts = shortcode_atts( array(
		'style' => ''
	), $atts);
	extract( $atts );
 ...
 * uk-accordion
 */
add_shortcode( 'uk-accordion' , function( $atts, $content ){
	$atts = shortcode_atts( array(
		'multiple' => ''
	), $atts);
	extract( $atts );
 ...
 * uk-tab
 */
add_shortcode( 'uk-tab' , function( $atts, $content ){
 ...
 * uk-grid
 */
add_shortcode( 'uk-grid' , function( $atts, $content ){
	static $kns_tab_num = 0;
 ...
 * uk-slideshow 
 */
add_shortcode( 'uk-slideshow' , function( $atts, $content ){
	static $kns_tab_num = 0;
...
<div class="uk-child-width-1-3@m uk-grid-small" uk-grid uk-height-match="target: > div > .uk-card; row: false">
    ...
    <?php endfor; wp_reset_postdata();  /* setup_postdata を使った後は、必ずリセットする */ ?>
</div>
<?php endfor; ?>
<?php
			$ret = ob_get_contents();
